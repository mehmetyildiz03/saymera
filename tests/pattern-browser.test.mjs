  ...
  for(const config of cases){
    ...
    try{
      ...
      for(const section of ['continue-sequence','missing-number','transfer-pattern']){
        ...
        await page.screenshot({path:'test-results/'+config.name+'-'+section+'.png',fullPage:false,animations:"disabled"});
      }
    ...
    }catch(error){await page.screenshot({path:'test-results/'+config.name+'-failure.png',fullPage:false,animations:"disabled"});throw error;}
