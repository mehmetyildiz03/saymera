...
const repDescriptions={
  build:'Nesne, parça ve hareketle yapıyı kur.',
  see:'Aynı ilişkiyi görsel modelde fark et.',
  symbol:'Gördüğünü sayı ve işaretlerle ifade et.',
  explain:'Stratejinin neden çalıştığını ayırt et.',
  transfer:'Aynı fikri yeni bir durumda kullan.'
};
const repReasons={
...
function focusDescription(focus){
  if(!focus) return 'Yeni bir kavram seçiliyor.';
  const ss=focus.state, pct=masteryPercent(ss), coverage=evidenceCoverage(ss);
  if(!ss.totalAttempts) return 'İlk karşılaşma: kavramı farklı türde görevlerle yoklayacağız.';
  const gap=representationGap(ss);
  if(coverage<5) return `${REPRESENTATION_META[gap.rep].label} penceresinde henüz yeterli kanıt yok. Bugünkü oturum aynı fikrin eksik bağlantılarını tamamlayacak.`;
  if(!ss.stable && pct>=60) return 'Kavram bugün güçlü görünüyor; kalıcı kabul edilmesi için başka bir zamanda kısa hatırlama kanıtı da gerekiyor.';
  if(ss.stable) return 'Bu kavram sağlam görünüyor. Sistem gerektiğinde kısa geri çağırmalarla dayanıklılığı koruyacak.';
  return `${REPRESENTATION_META[gap.rep].label} penceresi diğerlerine göre daha zayıf. Sıradaki görevler bu boşluğu hedefleyecek.`;
}
function renderHome(){
