...
const NEL_NUMBER_REP_LESSON_VERSION=1;
const NEL_NUMBER_REP_SECTIONS=['MİKTAR','RAKAM','SAYI SÖZCÜĞÜ','EŞLEŞTİR','HAYATA TAŞI'];
const NEL_NUMBER_WORDS=['','bir','iki','üç','dört','beş','altı','yedi','sekiz','dokuz','on'];
function nelNumberLessonQuantity(n,model='objects'){
  n=Math.max(1,Math.min(10,Number(n)||1));
  if(model==='fingers') return {type:'nel-number-quantity',model,n,left:Math.min(5,n),right:Math.max(0,n-5)};
  if(model==='ten-frame') return {type:'nel-number-quantity',model,n,cells:Array.from({length:10},(_,i)=>i<n)};
  if(model==='tally') return {type:'nel-number-quantity',model,n,groups:Math.floor(n/5),remainder:n%5};
  return {type:'nel-number-quantity',model:'objects',n,items:Array.from({length:n},(_,i)=>({id:'lesson-number-'+n+'-'+i,symbol:['●','■','★'][i%3]}))};
}
function nelNumberLessonCase(n){return {n,numberName:{kind:'number-name',speech:NEL_NUMBER_WORDS[n]},numeral:{kind:'numeral',value:String(n)},numberWord:{kind:'number-word',value:NEL_NUMBER_WORDS[n]},quantities:['objects','fingers','ten-frame','tally'].map(model=>nelNumberLessonQuantity(n,model))};}
const NEL_NUMBER_REP_LESSON_STEPS=[
  {id:'quantity-name-four',section:'MİKTAR',kind:'quantity-name',case:nelNumberLessonCase(4),title:'Önce miktar, sonra sayı adı.',body:'Dört nesneyi gör. Sonra bu miktarın sayı adını dinle.',result:'Bu miktarın söylenen sayı adı “dört”.'},
  {id:'quantity-numeral-four',section:'RAKAM',kind:'quantity-numeral',case:nelNumberLessonCase(4),title:'Miktarı rakamla bağla.',body:'Aynı dört miktarı şimdi 4 rakamıyla eşleştir.',result:'4 rakamı dört miktarını gösterebilir.'},
  {id:'same-five-models',section:'MİKTAR',kind:'models',case:nelNumberLessonCase(5),title:'Aynı beş, farklı görünüşler.',body:'Nesneler, parmaklar, onluk çerçeve ve çetele farklı görünür; hepsi aynı miktarı gösterebilir.',result:'Görünüş değişse de bu dört modelin hepsi beş miktarını gösteriyor.'},
  {id:'numeral-name-six',section:'RAKAM',kind:'numeral-name',case:nelNumberLessonCase(6),title:'6 rakamının sayı adını dinle.',body:'Rakam bir semboldür. Söylediğimiz sayı adı ise sestir.',result:'6 rakamı ile söylenen “altı” sayı adı aynı sayıyı anlatır.'},
  {id:'words-one-five',section:'SAYI SÖZCÜĞÜ',kind:'word-deck',range:[1,5],title:'Birden beşe sayı sözcüklerini tanı.',body:'Her kartta rakamı gör, yazılı sayı sözcüğünü incele ve sesini dinle.',result:'Rakam ile yazılı sayı sözcüğünün iki farklı gösterim olduğunu gördün.'},
  {id:'words-six-ten',section:'SAYI SÖZCÜĞÜ',kind:'word-deck',range:[6,10],title:'Altıdan ona sayı sözcüklerini tanı.',body:'Kartları tek tek aç; rakam, yazılı sözcük ve söylenen sayı adını birlikte gör.',result:'Altıdan ona kadar yazılı sayı sözcüklerini sayı adları ve rakamlarla bağladın.'},
...
function nelNumberQuantityVisual(v={}){
  const n=Number(v.n)||0,model=v.model||'objects';
  if(model==='fingers'){
    const hand=(count,label)=>'<div class="nel-number-hand" aria-label="'+esc(label)+'">'+Array.from({length:5},(_,i)=>'<i class="'+(i<count?'up':'')+'"></i>').join('')+'</div>';
    return '<div class="nel-number-quantity fingers" data-number-model="fingers" data-number-quantity="'+n+'">'+hand(Number(v.left)||0,'sol el')+hand(Number(v.right)||0,'sağ el')+'</div>';
  }
  if(model==='ten-frame') return '<div class="nel-number-quantity ten-frame" data-number-model="ten-frame" data-number-quantity="'+n+'">'+Array.from({length:10},(_,i)=>'<i class="'+(v.cells?.[i]?'filled':'')+'"></i>').join('')+'</div>';
  if(model==='tally'){
    const marks=Array.from({length:n},(_,i)=>'<i class="'+((i+1)%5===0?'fifth':'')+'"></i>').join('');
    return '<div class="nel-number-quantity tally" data-number-model="tally" data-number-quantity="'+n+'">'+marks+'</div>';
  }
  const items=v.items?.length?v.items:Array.from({length:n},(_,i)=>({symbol:['●','■','★'][i%3]}));
  return '<div class="nel-number-quantity objects" data-number-model="objects" data-number-quantity="'+n+'">'+items.map(item=>'<i>'+esc(item.symbol||'●')+'</i>').join('')+'</div>';
}
function nelNumberSpeechButton(numberName={},label='Sayı adını dinle'){return '<button type="button" class="nel-number-listen" data-rote-speech="'+esc(numberName.speech||'')+'">🔊 '+esc(label)+'</button>';}
function nelNumberLinkBuilderVisual(v={}){
  return '<div class="nel-number-link-builder">'+nelNumberQuantityVisual(v.quantity||{})+'<div class="nel-number-card-row">'+(v.options||[]).map(opt=>'<button type="button" data-nel-number-link="'+esc(opt.value)+'" class="nel-number-form-card numeral">'+esc(opt.label||'')+'</button>').join('')+'</div></div>';
}
...
function nelNumberFormMatchVisual(v={}){
  return '<div class="nel-number-form-match" data-number-name="'+esc(v.numberName?.id||'')+'" data-name-listened="false">'+
    ...
    '<div class="nel-number-match-quantity">'+nelNumberQuantityVisual(v.quantity||{})+'</div>'+
  '</div>';
}
function nelNumberEquivalentSetVisual(v={}){
  return '<div class="nel-number-equivalent-set">'+nelNumberSpeechButton(v.numberName)+
    '<div class="nel-number-form-card numeral static">'+esc(v.numeral?.value||'')+'</div>'+
    '<div class="nel-number-form-card word static">'+esc(v.numberWord?.value||'')+'</div>'+
...
function nelNumberWordDeck(range=[]){
  ...
  return '<div class="nel-number-word-deck">'+Array.from({length:to-from+1},(_,i)=>from+i).map(n=>
    '<article data-number-word-card="'+n+'"><strong>'+n+'</strong><b>'+esc(NEL_NUMBER_WORDS[n])+'</b>'+
    '<button type="button" data-rote-speech="'+esc(NEL_NUMBER_WORDS[n])+'" data-number-word-listen="'+n+'">🔊 Dinle</button>'+
    '<button type="button" class="nel-number-word-open" data-number-word-open="'+n+'" disabled>Bu kartı gördüm</button></article>'
  ).join('')+'</div>';
}
function nelNumberLessonCore(step){
  const c=step.case;
  if(step.kind==='quantity-name')return '<div class="nel-number-lesson-core">'+nelNumberQuantityVisual(c.quantities[0])+nelNumberSpeechButton(c.numberName)+'<button type="button" class="nel-number-confirm" id="nelNumberConfirm" disabled>Miktarı ve sayı adını bağladım</button><div class="nel-number-result" id="nelNumberResult">'+esc(step.result)+'</div></div>';
  if(step.kind==='quantity-numeral')return '<div class="nel-number-lesson-core">'+nelNumberQuantityVisual(c.quantities[2])+'<div class="nel-number-form-card numeral hero">'+esc(c.numeral.value)+'</div><button type="button" class="nel-number-confirm" id="nelNumberConfirm">Aynı sayıyı gösterdiklerini gördüm</button><div class="nel-number-result" id="nelNumberResult">'+esc(step.result)+'</div></div>';
  if(step.kind==='models')return '<div class="nel-number-lesson-core"><div class="nel-number-model-cycle" id="nelNumberModelCycle" data-model-index="0">'+nelNumberQuantityVisual(c.quantities[0])+'</div><button type="button" class="nel-number-confirm" id="nelNumberNextModel">Sonraki gösterim</button><p class="nel-number-help" id="nelNumberHelp">1 / 4 gösterim</p><div class="nel-number-result" id="nelNumberResult">'+esc(step.result)+'</div></div>';
  if(step.kind==='numeral-name')return '<div class="nel-number-lesson-core"><div class="nel-number-form-card numeral hero">'+esc(c.numeral.value)+'</div>'+nelNumberSpeechButton(c.numberName)+'<button type="button" class="nel-number-confirm" id="nelNumberConfirm" disabled>Rakam ile sesi bağladım</button><div class="nel-number-result" id="nelNumberResult">'+esc(step.result)+'</div></div>';
  if(step.kind==='word-deck')return '<div class="nel-number-lesson-core">'+nelNumberWordDeck(step.range)+'<p class="nel-number-help" id="nelNumberHelp">Her kartta önce sayı adını dinle, sonra kartı gördüğünü işaretle.</p><div class="nel-number-result" id="nelNumberResult">'+esc(step.result)+'</div></div>';
  if(step.kind==='word-quantity'){
    const options=[nelNumberLessonQuantity(7,'ten-frame'),nelNumberLessonQuantity(8,'ten-frame'),nelNumberLessonQuantity(9,'ten-frame')];
    return '<div class="nel-number-lesson-core"><div class="nel-number-form-card word hero">'+esc(c.numberWord.value)+'</div><div class="nel-number-lesson-choice-grid">'+options.map(o=>'<button type="button" data-number-lesson-quantity="'+o.n+'">'+nelNumberQuantityVisual(o)+'</button>').join('')+'</div><p class="nel-number-help" id="nelNumberHelp">Yazılı sayı sözcüğünün anlattığı miktarı seç.</p><div class="nel-number-result" id="nelNumberResult">'+esc(step.result)+'</div></div>';
  }
  if(step.kind==='four-way')return '<div class="nel-number-lesson-core"><div class="nel-number-four-way">'+nelNumberSpeechButton(c.numberName)+'<button type="button" data-number-four-part="numeral" class="nel-number-form-card numeral">'+esc(c.numeral.value)+'</button><button type="button" data-number-four-part="word" class="nel-number-form-card word">'+esc(c.numberWord.value)+'</button><button type="button" data-number-four-part="quantity" class="nel-number-quantity-button">'+nelNumberQuantityVisual(c.quantities[2])+'</button></div><p class="nel-number-help" id="nelNumberHelp">Sayı adını dinle ve üç görünür gösterimin hepsine dokun.</p><div class="nel-number-result" id="nelNumberResult">'+esc(step.result)+'</div></div>';
  if(step.kind==='mixed'){
...
function wireNelNumberLessonStep(step,next){
  ...
  const lessonRoot=$('.nel-number-lesson-core');
  bindNelRoteSpeech(lessonRoot);
  if(step.kind==='quantity-name'||step.kind==='numeral-name'){
    const confirm=$('#nelNumberConfirm'),listen=lessonRoot?.querySelector('[data-rote-speech]');
    listen?.addEventListener('click',()=>{if(confirm)confirm.disabled=false;});
    confirm?.addEventListener('click',reveal);
    return;
  }
  if(step.kind==='quantity-numeral'||step.kind==='transfer'){$('#nelNumberConfirm')?.addEventListener('click',reveal);return;}
  if(step.kind==='models'){
    const box=$('#nelNumberModelCycle'),button=$('#nelNumberNextModel'),models=step.case.quantities;
    button?.addEventListener('click',()=>{let i=Number(box.dataset.modelIndex||0)+1;if(i>=models.length){button.disabled=true;reveal();return;}box.dataset.modelIndex=String(i);box.innerHTML=nelNumberQuantityVisual(models[i]);$('#nelNumberHelp').textContent=(i+1)+' / '+models.length+' gösterim';if(i===models.length-1)button.textContent='Dört gösterimi tamamla';});
    return;
  }
  if(step.kind==='word-deck'){
    const root=$('.nel-number-word-deck'),seen=new Set();
    root?.querySelectorAll('[data-number-word-listen]').forEach(button=>button.addEventListener('click',()=>{const card=button.closest('article');card?.classList.add('listened');card?.querySelector('[data-number-word-open]')?.removeAttribute('disabled');}));
    root?.querySelectorAll('[data-number-word-open]').forEach(button=>button.addEventListener('click',()=>{if(button.disabled)return;seen.add(button.dataset.numberWordOpen);button.closest('article')?.classList.add('seen');button.disabled=true;$('#nelNumberHelp').textContent=seen.size+' / '+root.querySelectorAll('article').length+' kart görüldü';if(seen.size===root.querySelectorAll('article').length)reveal();}));
    return;
  }
  if(step.kind==='word-quantity'){
    $$('[data-number-lesson-quantity]').forEach(button=>button.addEventListener('click',()=>{if(Number(button.dataset.numberLessonQuantity)!==step.case.n){$('#nelNumberHelp').textContent='Sözcüğü sesli söyle ve miktarı yeniden karşılaştır.';return;}button.classList.add('selected');reveal();}));
    return;
  }
  if(step.kind==='four-way'){
    const seen=new Set();
    lessonRoot?.querySelector('[data-rote-speech]')?.addEventListener('click',()=>{seen.add('name');if(seen.size===4)reveal();});
    $$('[data-number-four-part]').forEach(button=>button.addEventListener('click',()=>{seen.add(button.dataset.numberFourPart);button.classList.add('selected');if(seen.size===4)reveal();}));
    return;
  }
  if(step.kind==='mixed'){
    lessonRoot?.querySelector('[data-rote-speech]')?.addEventListener('click',()=>$$('[data-number-mixed]').forEach(button=>button.disabled=false));
    $$('[data-number-mixed]').forEach(button=>button.addEventListener('click',()=>{if(button.disabled)return;if(Number(button.dataset.numberMixed)!==step.case.n){$('#nelNumberHelp').textContent='Hedef sayı adını yeniden dinle; rakam, sözcük ve miktarın üçünü birlikte kontrol et.';return;}button.classList.add('selected');reveal();}));
    return;
  }
}
function completeNelNumberLessonStep(skill,index){
  const ss=ensureSkillState(state,skill.id),lc=ss.learningCycle,next=index+1;lc.lessonStepIndex=Math.max(lc.lessonStepIndex||0,next);lc.lessonVersion=NEL_NUMBER_REP_LESSON_VERSION;
  if(next>=NEL_NUMBER_REP_LESSON_STEPS.length){lc.lessonTaughtAt=lc.lessonTaughtAt||Date.now();saveState();session.planIndex++;loadPlanItem();return;}saveState();session.lessonStepIndex=next;renderNelNumberRepresentationsLessonStep(skill,next);
}
function renderNelNumberRepresentationsLessonStep(skill,index=null){
  const ss=ensureSkillState(state,skill.id),saved=Math.min(NEL_NUMBER_REP_LESSON_STEPS.length-1,Math.max(0,ss.learningCycle?.lessonStepIndex||0));
  const at=index==null?(session?.lessonReplayStep!=null?Math.min(NEL_NUMBER_REP_LESSON_STEPS.length-1,Math.max(0,Number(session.lessonReplayStep)||0)):(session?.lessonReplay?0:saved)):index;
  const step=NEL_NUMBER_REP_LESSON_STEPS[at];session.lessonStepIndex=at;currentQuestion=null;renderPracticeHeader(skill);$('#practiceMode').textContent='KEŞFET';$('#practiceMode').dataset.mode='teach';$('#practiceCounter').textContent=step.section+' • '+(at+1)+' / '+NEL_NUMBER_REP_LESSON_STEPS.length;$('#practiceProgress').style.width=Math.round((at+1)/NEL_NUMBER_REP_LESSON_STEPS.length*100)+'%';
  $('#practiceContent').innerHTML='<div class="nel-number-lesson-stage" data-nel-number-step="'+esc(step.id)+'">'+nelNumberRepSectionTrack(step)+'<div class="lesson-step-copy"><span class="lesson-kicker">'+esc(step.section)+' · '+(at+1)+' / '+NEL_NUMBER_REP_LESSON_STEPS.length+'</span><h2>'+esc(step.title)+'</h2><p>'+esc(step.body)+'</p></div><div class="nel-number-lesson-visual">'+nelNumberLessonCore(step)+'</div><div class="lesson-step-actions"><button type="button" class="response-submit lesson-next-button" id="nelNumberLessonNext" disabled>'+(at===NEL_NUMBER_REP_LESSON_STEPS.length-1?'Sayı hissi yoluna devam':'Sonraki keşif')+' <b>→</b></button></div></div>';
  const next=$('#nelNumberLessonNext');wireNelNumberLessonStep(step,next);next?.addEventListener('click',()=>completeNelNumberLessonStep(skill,at));
}


const NEL_NUMERAL_FORMATION_LESSON_VERSION=1;
const NEL_NUMERAL_FORMATION_SECTIONS=['HAMURLA KUR','YOLU İZLE','İKİ RAKAM'];
const NEL_NUMERAL_FORMATION_LESSON_STEPS=[
  {id:'form-one',section:'HAMURLA KUR',kind:'material',n:1,title:'1 rakamının biçimini kur.',body:'Sanal hamur izini geniş yolun üzerine yay.',result:'1 rakamının biçimini oluşturdun.'},
  {id:'trace-two',section:'YOLU İZLE',kind:'guided',n:2,title:'2 rakamının yolunu izle.',body:'Geniş yol boyunca ilerle ve 2 rakamını tamamla.',result:'2 rakamını baştan sona oluşturdun.'},
  {id:'trace-three',section:'YOLU İZLE',kind:'guided',n:3,title:'3 rakamının iki kıvrımını oluştur.',body:'Yolu takip ederek üst ve alt kıvrımı birbirine bağla.',result:'3 rakamının iki kıvrımı tek biçimde birleşti.'},
  {id:'trace-four',section:'YOLU İZLE',kind:'guided',n:4,title:'4 rakamının parçalarını birleştir.',body:'Rakamın köşe ve çizgilerinden geçerek biçimi tamamla.',result:'4 rakamının parçaları birlikte tanınan biçimi oluşturdu.'},
  {id:'playdough-five',section:'HAMURLA KUR',kind:'material',n:5,title:'5 rakamını hamur iziyle kur.',body:'Parmağınla veya kaleminle yolu takip ederek 5 rakamını oluştur.',result:'5 rakamını malzeme oluşturur gibi biçimlendirdin.'},
  {id:'trace-six',section:'YOLU İZLE',kind:'guided',n:6,title:'6 rakamının kıvrımını tamamla.',body:'Üstten aşağı inen yolu ve alt halkayı birleştir.',result:'6 rakamının kıvrımı tamamlandı.'},
  {id:'trace-seven',section:'YOLU İZLE',kind:'guided',n:7,title:'7 rakamının iki yönünü bağla.',body:'Üst çizgiyi ve aşağı inen çizgiyi aynı rakamda birleştir.',result:'7 rakamının iki bölümü bir araya geldi.'},
  {id:'trace-eight',section:'YOLU İZLE',kind:'guided',n:8,title:'8 rakamının iki halkasını oluştur.',body:'Üst ve alt halkadan geçerek 8 rakamını tamamla.',result:'8 rakamının iki halkası tamamlandı.'},
  {id:'trace-nine',section:'YOLU İZLE',kind:'guided',n:9,title:'9 rakamının halkasını ve kuyruğunu kur.',body:'Önce halkayı, sonra aşağı uzanan yolu tamamla.',result:'9 rakamının biçimi tamamlandı.'},
  {id:'write-ten',section:'İKİ RAKAM',kind:'guided-context',n:10,title:'10 iki rakamla yazılır.',body:'On puanı kaydetmek için 1 ve 0 yollarının ikisini de tamamla.',result:'1 ve 0 yan yana gelince 10 rakamı oluştu.'}
];
function nelNumeralFormationSectionTrack(step){
  return '<div class="nel-numeral-section-track">'+NEL_NUMERAL_FORMATION_SECTIONS.map(name=>'<span class="'+(name===step.section?'active':'')+'">'+esc(name)+'</span>').join('')+'</div>';
}
function nelNumeralGuidesAttr(guides=[]){ return esc(encodeURIComponent(JSON.stringify(guides))); }
function nelNumeralGuideMarkup(guides=[]){
  return guides.map(guide=>'<polyline class="nel-numeral-guide" data-numeral-guide="'+esc(guide.id)+'" points="'+guide.points.map(point=>point.join(',')).join(' ')+'"></polyline>').join('');
}
function nelNumeralContextBanner(context={}){
  if(context?.kind!=='game-score')return '';
  return '<div class="nel-numeral-context-banner"><span>OYUN SKORU</span><strong>Skorunu rakamla kaydet</strong></div>';
}
function nelNumeralBoardVisual(v={},material=false){
  const guides=v.guides||[],mode=v.mode||'guided',showGuide=v.guideVisible!==false;
  const quantity=v.quantity?'<div class="nel-numeral-quantity-link"><span>BU MİKTAR</span>'+nelNumberQuantityVisual(v.quantity)+'</div>':'';
  return '<div class="nel-numeral-board '+(material?'material-mode':'')+'" data-numeral="'+esc(v.numeral||'')+'" data-numeral-mode="'+esc(mode)+'" data-numeral-guides="'+nelNumeralGuidesAttr(guides)+'" data-numeral-ready="false">'+
    nelNumeralContextBanner(v.context||{})+quantity+
    '<div class="nel-numeral-paper"><svg class="nel-numeral-svg" viewBox="0 0 100 100" role="img" aria-label="Rakam oluşturma alanı">'+
      (showGuide?'<g class="nel-numeral-guides">'+nelNumeralGuideMarkup(guides)+'</g>':'')+
      '<g class="nel-numeral-strokes" data-numeral-strokes></g></svg>'+
      '<div class="nel-numeral-start-note">'+(material?'Hamur izini yolun üzerine yay':'Parmağınla ya da kaleminle çiz')+'</div>'+
    '</div>'+
    '<div class="nel-numeral-board-actions"><button type="button" data-numeral-clear>Temizle</button><span data-numeral-board-status>Rakamı oluştur.</span></div>'+
  '</div>';
}
function nelNumeralMaterialVisual(v={}){ return nelNumeralBoardVisual({...v,mode:v.mode||'material',guideVisible:true},true); }
function nelNumeralWrittenRecordVisual(v={}){
  return '<div class="nel-numeral-written-record"><div class="nel-numeral-record-symbol">'+esc(v.numeral||'')+'</div>'+(v.quantity?nelNumberQuantityVisual(v.quantity):'')+'</div>';
}
function nelNumeralGuidesFromRoot(root){
  try{return JSON.parse(decodeURIComponent(root?.dataset.numeralGuides||''));}catch{return [];}
}
function nelNumeralDist(a,b){return Math.hypot(Number(a.x)-Number(b.x),Number(a.y)-Number(b.y));}
function nelNumeralPointSegmentDistance(point,a,b){
  const vx=b.x-a.x,vy=b.y-a.y,wx=point.x-a.x,wy=point.y-a.y,den=vx*vx+vy*vy;
  if(!den)return nelNumeralDist(point,a);
  const t=Math.max(0,Math.min(1,(wx*vx+wy*vy)/den));
  return Math.hypot(point.x-(a.x+t*vx),point.y-(a.y+t*vy));
}
function nelNumeralDistanceToGuide(point,guide){
  const pts=(guide.points||[]).map(([x,y])=>({x:Number(x),y:Number(y)}));
  if(pts.length<2)return pts.length?nelNumeralDist(point,pts[0]):Infinity;
  let best=Infinity;
  for(let i=1;i<pts.length;i++)best=Math.min(best,nelNumeralPointSegmentDistance(point,pts[i-1],pts[i]));
  return best;
}
function nelNumeralEvaluate(root){
  const guides=nelNumeralGuidesFromRoot(root),samples=root?._nelNumeralSamples||[];
  if(!root||!guides.length||!samples.length)return {pass:false,coverage:0,onPathRatio:0,guideRatios:[]};
  const guideRatios=guides.map(guide=>{
    const tolerance=Number(guide.tolerance)||14,points=guide.points||[];
    const hit=points.filter(([x,y])=>samples.some(sample=>Math.hypot(sample.x-Number(x),sample.y-Number(y))<=tolerance)).length;
    return points.length?hit/points.length:0;
  });
  const onPath=samples.filter(sample=>guides.some(guide=>nelNumeralDistanceToGuide(sample,guide)<=(Number(guide.tolerance)||14)*1.2)).length;
  const onPathRatio=onPath/samples.length;
  const coverage=guideRatios.length?Math.min(...guideRatios):0;
  const requiredSamples=Math.max(10,guides.reduce((sum,guide)=>sum+(guide.points?.length||0),0)*2);
  const pass=samples.length>=requiredSamples && guides.every((guide,index)=>guideRatios[index]>=(Number(guide.minimumHitRatio)||.78)) && onPathRatio>=.5;
  root.dataset.numeralReady=pass?'true':'false';
  root.dataset.numeralCoverage=String(Math.round(coverage*100));
  root.dataset.numeralOnPath=String(Math.round(onPathRatio*100));
  return {pass,coverage,onPathRatio,guideRatios};
}
function nelNumeralSvgPoint(svg,event){
...
function nelNumeralAddSample(root,point){
  root._nelNumeralSamples||=[];
  const prev=root._nelNumeralSamples.at(-1);
  if(prev&&root._nelNumeralStrokeOpen){
    const d=Math.max(0.001,nelNumeralDist(prev,point)),steps=Math.max(1,Math.ceil(d/2.2));
    for(let i=1;i<=steps;i++)root._nelNumeralSamples.push({x:prev.x+(point.x-prev.x)*(i/steps),y:prev.y+(point.y-prev.y)*(i/steps)});
  }else root._nelNumeralSamples.push(point);
}
function nelNumeralStrokePath(points=[]){
  return points.map((point,index)=>(index?'L':'M')+point.x.toFixed(1)+' '+point.y.toFixed(1)).join(' ');
}
function bindNelNumeralBoard(root,onChange,blocked=()=>false){
  if(!root||root.dataset.numeralBound==='true')return;
  root.dataset.numeralBound='true';root._nelNumeralSamples=[];root._nelNumeralStrokes=[];root._nelNumeralStrokeOpen=false;
  const svg=root.querySelector('.nel-numeral-svg'),layer=root.querySelector('[data-numeral-strokes]'),status=root.querySelector('[data-numeral-board-status]');
  if(!svg||!layer)return;
  let activePointer=null,current=null,path=null;
  const refresh=()=>{
    const result=nelNumeralEvaluate(root);
    if(status)status.textContent=result.pass?'Rakam hazır.':'Yolun eksik kalan bölümlerini tamamla.';
    root.classList.toggle('ready',result.pass);
    onChange?.(result);
  };
  const begin=event=>{
    if(blocked())return;
    event.preventDefault();activePointer=event.pointerId;svg.setPointerCapture?.(event.pointerId);
    current=[];root._nelNumeralStrokes.push(current);root._nelNumeralStrokeOpen=false;
    path=document.createElementNS('http://www.w3.org/2000/svg','path');path.setAttribute('class','nel-numeral-user-stroke');layer.appendChild(path);
    const point=nelNumeralSvgPoint(svg,event);current.push(point);nelNumeralAddSample(root,point);root._nelNumeralStrokeOpen=true;path.setAttribute('d',nelNumeralStrokePath(current));
  };
  const move=event=>{
    if(activePointer!==event.pointerId||blocked())return;
    event.preventDefault();const point=nelNumeralSvgPoint(svg,event),last=current?.at(-1);
    if(last&&nelNumeralDist(last,point)<.7)return;
    current.push(point);nelNumeralAddSample(root,point);path?.setAttribute('d',nelNumeralStrokePath(current));
  };
  const end=event=>{
    if(activePointer!==event.pointerId)return;
    event.preventDefault();root._nelNumeralStrokeOpen=false;activePointer=null;svg.releasePointerCapture?.(event.pointerId);refresh();
  };
  ...
  root.querySelector('[data-numeral-clear]')?.addEventListener('click',()=>{
    if(blocked())return;root._nelNumeralSamples=[];root._nelNumeralStrokes=[];root._nelNumeralStrokeOpen=false;layer.innerHTML='';root.dataset.numeralReady='false';root.dataset.numeralCoverage='0';root.classList.remove('ready');if(status)status.textContent='Rakamı yeniden oluştur.';onChange?.({pass:false,coverage:0,onPathRatio:0,guideRatios:[]});
  });
}
function nelNumeralReadBoard(root){
  if(!root||root.dataset.numeralReady!=='true')return null;
  return 'formed-'+root.dataset.numeral;
}
function nelNumeralFormationLessonVisual(step){
  const sample=numeralFormationCaseFor(step.n);
  const context=step.kind==='guided-context'?{kind:'game-score',value:sample.n,label:'oyun skoru'}:null;
  const visual={type:step.kind==='material'?'nel-numeral-material-form':'nel-numeral-draw-board',numeral:sample.numeral,guides:sample.guides,quantity:sample.quantity,guideVisible:true,mode:step.kind==='material'?'material':step.kind==='guided-context'?'guided-context':'guided',context};
  return step.kind==='material'?nelNumeralMaterialVisual(visual):nelNumeralBoardVisual(visual);
}
function wireNelNumeralFormationLessonStep(step,next){
  const root=$('.nel-numeral-board'),result=$('#nelNumeralResult');
  bindNelNumeralBoard(root,state=>{
    if(!state.pass)return;
    result?.classList.add('revealed');if(next)next.disabled=false;
  });
}
function completeNelNumeralFormationLessonStep(skill,index){
  const ss=ensureSkillState(state,skill.id),lc=ss.learningCycle,next=index+1;lc.lessonStepIndex=Math.max(lc.lessonStepIndex||0,next);lc.lessonVersion=NEL_NUMERAL_FORMATION_LESSON_VERSION;
  if(next>=NEL_NUMERAL_FORMATION_LESSON_STEPS.length){lc.lessonTaughtAt=lc.lessonTaughtAt||Date.now();saveState();session.planIndex++;loadPlanItem();return;}saveState();session.lessonStepIndex=next;renderNelNumeralFormationLessonStep(skill,next);
}
function renderNelNumeralFormationLessonStep(skill,index=null){
  const ss=ensureSkillState(state,skill.id),saved=Math.min(NEL_NUMERAL_FORMATION_LESSON_STEPS.length-1,Math.max(0,ss.learningCycle?.lessonStepIndex||0));
  const at=index==null?(session?.lessonReplayStep!=null?Math.min(NEL_NUMERAL_FORMATION_LESSON_STEPS.length-1,Math.max(0,Number(session.lessonReplayStep)||0)):(session?.lessonReplay?0:saved)):index;
  const step=NEL_NUMERAL_FORMATION_LESSON_STEPS[at];session.lessonStepIndex=at;currentQuestion=null;renderPracticeHeader(skill);$('#practiceMode').textContent='KEŞFET';$('#practiceMode').dataset.mode='teach';$('#practiceCounter').textContent=step.section+' • '+(at+1)+' / '+NEL_NUMERAL_FORMATION_LESSON_STEPS.length;$('#practiceProgress').style.width=Math.round((at+1)/NEL_NUMERAL_FORMATION_LESSON_STEPS.length*100)+'%';
  $('#practiceContent').innerHTML='<div class="nel-numeral-lesson-stage" data-nel-numeral-step="'+esc(step.id)+'">'+nelNumeralFormationSectionTrack(step)+'<div class="lesson-step-copy"><span class="lesson-kicker">'+esc(step.section)+' · '+(at+1)+' / '+NEL_NUMERAL_FORMATION_LESSON_STEPS.length+'</span><h2>'+esc(step.title)+'</h2><p>'+esc(step.body)+'</p></div><div class="nel-numeral-lesson-visual">'+nelNumeralFormationLessonVisual(step)+'<div class="nel-numeral-result" id="nelNumeralResult">'+esc(step.result)+'</div></div><div class="lesson-step-actions"><button type="button" class="response-submit lesson-next-button" id="nelNumeralLessonNext" disabled>'+(at===NEL_NUMERAL_FORMATION_LESSON_STEPS.length-1?'Sayı hissi yoluna devam':'Sonraki keşif')+' <b>→</b></button></div></div>';
  const next=$('#nelNumeralLessonNext');wireNelNumeralFormationLessonStep(step,next);next?.addEventListener('click',()=>completeNelNumeralFormationLessonStep(skill,at));
}


const NEL_QUANTITY_COMPARE_LESSON_VERSION=1;
const NEL_QUANTITY_COMPARE_SECTIONS=['EŞLEŞTİR','DAHA ÇOK','DAHA AZ','AYNI','ANLAT','HAYATA TAŞI'];
const NEL_QUANTITY_COMPARE_LESSON_STEPS=[
  {id:'pair-same-four',section:'EŞLEŞTİR',kind:'pair',caseId:'same-4-spread',title:'İki küme farklı görünse de aynı sayıda olabilir.',body:'Her sol nesneyi bir sağ nesneyle eşleştir. Eşsiz nesne kalıyor mu bak.',result:'Her nesnenin bir eşi var; iki küme aynı sayıda.'},
  {id:'pair-more-five-three',section:'EŞLEŞTİR',kind:'pair',caseId:'left-more-5-3',title:'Eşsiz kalan nesne daha çok olan kümeyi gösterir.',body:'İki kümeyi bire bir eşleştir ve hangi tarafta nesne kaldığını gör.',result:'Sol kümede eşsiz nesne kaldı; sol kümede daha çok nesne var.'},
  {id:'see-right-more',section:'DAHA ÇOK',kind:'side-choice',caseId:'right-more-4-6',title:'Daha çok olanı miktara bakarak bul.',body:'Nesnelerin aralığına değil, kaç nesne olduğuna karar ver.',result:'Sağ kümede daha çok nesne var.'},
  {id:'resist-size-spacing',section:'DAHA ÇOK',kind:'side-choice',caseId:'left-more-9-6',title:'Büyük görünen nesneler daha çok demek değildir.',body:'Bir taraftaki nesneler daha büyük veya daha yaygın görünebilir. Miktarı karşılaştır.',result:'Sol kümenin nesneleri daha küçük görünse de miktarı daha çok.'},
  {id:'fewer-language',section:'DAHA AZ',kind:'left-relation',caseId:'right-more-5-8',title:'Daha az sayıda olan kümeyi söyle.',body:'Sol kümeyi sağ kümeyle karşılaştır ve uygun miktar cümlesini seç.',result:'Sol kümede daha az sayıda nesne var.'},
  {id:'less-language',section:'DAHA AZ',kind:'language-pair',caseId:'right-more-6-10',title:'Aynı miktar ilişkisini iki şekilde söyleyebiliriz.',body:'Sol kümede daha az sayıda nesne var. “Daha az” sözü de burada aynı küçük miktarı anlatır.',result:'İki söyleyiş de sol kümenin miktarının daha az olduğunu anlatıyor.'},
  {id:'same-ten',section:'AYNI',kind:'left-relation',caseId:'same-10',title:'Düzen değişse de iki küme aynı sayıda olabilir.',body:'İki kümenin yerleşimi farklı. Miktar ilişkisini seç.',result:'Her iki kümede de aynı miktar var.'},
  {id:'reverse-relation',section:'DAHA AZ',kind:'left-relation',caseId:'right-more-4-6',title:'Bir küme daha çoksa öteki daha azdır.',body:'Sol kümenin sağ kümeye göre miktarını söyle.',result:'Sağ küme daha çok olduğu için sol küme daha az sayıda.'},
  {id:'explain-leftover',section:'ANLAT',kind:'proof',caseId:'left-more-10-7',title:'Neden daha çok olduğunu eşleşmeyen nesne gösterir.',body:'Bire bir eşleşen çiftlere ve dışarıda kalan nesnelere bak.',result:'Sol kümede eşsiz nesne kaldığı için sol kümede daha çok nesne var.'},
  {id:'object-graph',section:'HAYATA TAŞI',kind:'graph-choice',caseId:'right-more-6-10',title:'Gerçek nesne grafiğinde de aynı karşılaştırmayı yap.',body:'İki nesne sırasını karşılaştır. Sıra uzunluğuna değil nesne sayısına bak.',result:'Gerçek nesne sıralarında da miktarı bire bir karşılaştırabiliriz.'}
];
function nelQuantityCompareSectionTrack(step){
  return '<div class="nel-quantity-section-track">'+NEL_QUANTITY_COMPARE_SECTIONS.map(name=>'<span class="'+(name===step.section?'active':'')+'">'+esc(name)+'</span>').join('')+'</div>';
}
function nelQuantityToneClass(tone){return 'tone-'+(['blue','red','green','yellow'].includes(tone)?tone:'blue');}
function nelQuantitySetMarkup(set={},side='left',{interactive=false,label=true}={}){
  const items=set.items||[],layout=set.layout||'compact-row',scale=Math.max(.72,Math.min(1.3,Number(set.tokenScale)||1));
  return '<div class="nel-quantity-set '+esc(layout)+'" data-quantity-set="'+esc(side)+'" style="--quantity-scale:'+scale+'">'+
    (label?'<small>'+(side==='left'?'SOL KÜME':'SAĞ KÜME')+'</small>':'')+
    '<div class="nel-quantity-items">'+items.map((item,index)=>{
      const tag=interactive?'button':'span';
      return '<'+tag+(interactive?' type="button"':'')+' class="nel-quantity-token '+nelQuantityToneClass(item.tone)+'" data-quantity-item="'+esc(item.id)+'" data-quantity-side="'+esc(side)+'" data-quantity-index="'+index+'" aria-label="'+(side==='left'?'sol':'sağ')+' küme nesnesi">'+esc(item.symbol||'●')+'</'+tag+'>';
    }).join('')+'</div></div>';
}
function nelQuantitySetPairVisual(v={}){
  return '<div class="nel-quantity-set-pair">'+nelQuantitySetMarkup(v.left,'left')+nelQuantitySetMarkup(v.right,'right')+'</div>';
}
function nelQuantityPairBuilderVisual(v={}){
  const leftN=Number(v.left?.count||v.left?.items?.length||0),rightN=Number(v.right?.count||v.right?.items?.length||0);
  return '<div class="nel-quantity-pair-builder" data-left-n="'+leftN+'" data-right-n="'+rightN+'" data-relation="'+esc(v.relation||'')+'" data-paired-count="0">'+
    '<p class="nel-quantity-pair-instruction">Soldan bir nesne, sonra sağdan bir nesne seç.</p>'+
    '<div class="nel-quantity-set-pair">'+nelQuantitySetMarkup(v.left,'left',{interactive:true})+nelQuantitySetMarkup(v.right,'right',{interactive:true})+'</div>'+
    '<div class="nel-quantity-pair-summary" data-quantity-pair-summary>Henüz eşleştirme yok.</div>'+
  '</div>';
}
function nelQuantityRelationChoiceVisual(v={}){
  const options=(v.options||[]).map(String);
  return '<div class="nel-quantity-relation-choice">'+nelQuantitySetPairVisual(v)+
    '<div class="nel-quantity-relation-buttons">'+options.map(value=>'<button type="button" data-quantity-relation-choice="'+esc(value)+'">'+esc(value)+'</button>').join('')+'</div></div>';
}
function nelQuantityPairedProofVisual(v={}){
  const left=v.left||{},right=v.right||{},leftItems=left.items||[],rightItems=right.items||[],max=Math.max(leftItems.length,rightItems.length),paired=Math.min(leftItems.length,rightItems.length);
  return '<div class="nel-quantity-proof">'+
    '<div class="nel-quantity-proof-head"><span>SOL</span><span>BİRE BİR</span><span>SAĞ</span></div>'+
    '<div class="nel-quantity-proof-rows">'+Array.from({length:max},(_,i)=>{
      const li=leftItems[i],ri=rightItems[i],isPair=i<paired;
      const leftHtml=li?'<span class="nel-quantity-token '+nelQuantityToneClass(li.tone)+' '+(!isPair?'unmatched':'')+'">'+esc(li.symbol||'●')+'</span>':'<i></i>';
      const rightHtml=ri?'<span class="nel-quantity-token '+nelQuantityToneClass(ri.tone)+' '+(!isPair?'unmatched':'')+'">'+esc(ri.symbol||'●')+'</span>':'<i></i>';
      return '<div class="nel-quantity-proof-row">'+leftHtml+'<b>'+(isPair?'↔':'•')+'</b>'+rightHtml+'</div>';
    }).join('')+'</div>'+
    '<p>Eşleşmeyen nesne kalan tarafın miktarı daha çoktur.</p></div>';
}
function nelQuantityObjectGraphVisual(v={}){
  const row=(set,side)=>'<div class="nel-quantity-graph-row"><small>'+(side==='left'?'1. SIRA':'2. SIRA')+'</small><div>'+((set?.items)||[]).map(item=>'<span class="nel-quantity-graph-object '+nelQuantityToneClass(item.tone)+'">'+esc(item.symbol||'●')+'</span>').join('')+'</div></div>';
  return '<div class="nel-quantity-object-graph" data-quantity-context="'+esc(v.context||'real-object-graph')+'"><span class="nel-quantity-graph-kicker">GERÇEK NESNE GRAFİĞİ</span>'+row(v.left,'left')+row(v.right,'right')+
    ((v.options||[]).length?'<div class="nel-quantity-relation-buttons">'+v.options.map(value=>'<button type="button" data-quantity-relation-choice="'+esc(value)+'">'+esc(value)+'</button>').join('')+'</div>':'')+'</div>';
}
function bindNelQuantityRelation(root,onChange,blocked=()=>false){
  root?.querySelectorAll('[data-quantity-relation-choice]').forEach(button=>button.addEventListener('click',()=>{
    if(blocked())return;
    root.querySelectorAll('[data-quantity-relation-choice]').forEach(x=>x.classList.remove('selected'));
    button.classList.add('selected');
    root.dataset.quantityRelationSelected=button.dataset.quantityRelationChoice;
    onChange?.();
  }));
}
function nelQuantityRelationRead(root){return root?.dataset.quantityRelationSelected||null;}
function bindNelQuantityPairBuilder(root,onChange,blocked=()=>false){
  if(!root)return;
  let selectedLeft=null,selectedRight=null,pairIndex=0;
  const minPairs=Math.min(Number(root.dataset.leftN)||0,Number(root.dataset.rightN)||0);
  const summary=root.querySelector('[data-quantity-pair-summary]');
  const refresh=()=>{
    const paired=root.querySelectorAll('[data-quantity-side="left"].paired').length;
    root.dataset.pairedCount=String(paired);
    if(paired>=minPairs&&minPairs>0){
      root.dataset.pairComplete='true';
      root.querySelectorAll('[data-quantity-item]:not(.paired)').forEach(item=>item.classList.add('unmatched'));
      const l=Number(root.dataset.leftN)||0,r=Number(root.dataset.rightN)||0;
      if(summary)summary.textContent=l===r?'Bütün nesneler eşleşti; eşsiz nesne kalmadı.':(l>r?'Sol kümede eşsiz nesne kaldı.':'Sağ kümede eşsiz nesne kaldı.');
    }else if(summary) summary.textContent=paired+' / '+minPairs+' çift eşleşti.';
    onChange?.();
  };
  const pairIfReady=()=>{
    if(!selectedLeft||!selectedRight)return;
    pairIndex+=1;
    for(const node of [selectedLeft,selectedRight]){
      node.classList.remove('selected');node.classList.add('paired');node.dataset.pairIndex=String(pairIndex);node.disabled=true;
    }
    selectedLeft=null;selectedRight=null;refresh();
  };
  root.querySelectorAll('[data-quantity-item]').forEach(button=>button.addEventListener('click',()=>{
    if(blocked()||button.disabled||root.dataset.pairComplete==='true')return;
    const side=button.dataset.quantitySide;
    if(side==='left'){
      root.querySelectorAll('[data-quantity-side="left"].selected').forEach(x=>x.classList.remove('selected'));
      selectedLeft=button;button.classList.add('selected');
    }else{
      root.querySelectorAll('[data-quantity-side="right"].selected').forEach(x=>x.classList.remove('selected'));
      selectedRight=button;button.classList.add('selected');
    }
    pairIfReady();if(!selectedLeft||!selectedRight)onChange?.();
  }));
}
function nelQuantityPairRead(root){
  if(!root||root.dataset.pairComplete!=='true')return null;
  return 'paired|'+root.dataset.leftN+'|'+root.dataset.rightN+'|'+root.dataset.relation;
}
function nelQuantityLessonSideOptions(x){
  const correct=x.language.sideAnswer;
  const options=[correct,'İki küme aynı sayıda.','Sol kümede daha çok nesne var.','Sağ kümede daha çok nesne var.'];
  return [...new Set(options)].slice(0,4);
}
function nelQuantityLessonRelationOptions(x){
  return [...new Set([x.language.relationLabel,'Sol küme sağ kümeyle aynı sayıda.','Sol kümede daha çok nesne var.','Sol kümede daha az sayıda nesne var.'])].slice(0,4);
}
function nelQuantityLessonCore(step){
  const x=quantityCompareCaseFor(step.caseId);
  if(step.kind==='pair') return '<div class="nel-quantity-lesson-core">'+nelQuantityPairBuilderVisual({left:x.left,right:x.right,relation:x.relation})+'<div class="nel-quantity-lesson-result" id="nelQuantityResult">'+esc(step.result)+'</div></div>';
  if(step.kind==='side-choice') return '<div class="nel-quantity-lesson-core">'+nelQuantityRelationChoiceVisual({left:x.left,right:x.right,options:nelQuantityLessonSideOptions(x)})+'<p class="nel-quantity-help" id="nelQuantityHelp">Miktarı karşılaştır ve bir cümle seç.</p><div class="nel-quantity-lesson-result" id="nelQuantityResult">'+esc(step.result)+'</div></div>';
  if(step.kind==='left-relation') return '<div class="nel-quantity-lesson-core">'+nelQuantityRelationChoiceVisual({left:x.left,right:x.right,options:nelQuantityLessonRelationOptions(x)})+'<p class="nel-quantity-help" id="nelQuantityHelp">Sol kümenin sağ kümeye göre miktarını söyle.</p><div class="nel-quantity-lesson-result" id="nelQuantityResult">'+esc(step.result)+'</div></div>';
  if(step.kind==='language-pair') return '<div class="nel-quantity-lesson-core">'+nelQuantitySetPairVisual({left:x.left,right:x.right})+'<div class="nel-quantity-language-pair"><button type="button" data-quantity-language="fewer">daha az sayıda</button><button type="button" data-quantity-language="less">daha az</button></div><p class="nel-quantity-help" id="nelQuantityHelp">İki söyleyişe de dokun.</p><div class="nel-quantity-lesson-result" id="nelQuantityResult">'+esc(step.result)+'</div></div>';
  if(step.kind==='proof') return '<div class="nel-quantity-lesson-core">'+nelQuantityPairedProofVisual({left:x.left,right:x.right})+'<button type="button" class="nel-quantity-confirm" id="nelQuantityConfirm">Eşsiz kalan nesneyi gördüm</button><div class="nel-quantity-lesson-result" id="nelQuantityResult">'+esc(step.result)+'</div></div>';
  return '<div class="nel-quantity-lesson-core">'+nelQuantityObjectGraphVisual({left:x.left,right:x.right,options:nelQuantityLessonSideOptions(x),context:'real-object-graph'})+'<p class="nel-quantity-help" id="nelQuantityHelp">İki nesne sırasını karşılaştır.</p><div class="nel-quantity-lesson-result" id="nelQuantityResult">'+esc(step.result)+'</div></div>';
}
function wireNelCompareQuantitiesLessonStep(step,next){
  const core=$('.nel-quantity-lesson-core'),result=$('#nelQuantityResult'),help=$('#nelQuantityHelp'),x=quantityCompareCaseFor(step.caseId);
  const reveal=()=>{result?.classList.add('revealed');if(next)next.disabled=false;};
  if(step.kind==='pair'){
    const root=core?.querySelector('.nel-quantity-pair-builder');
    bindNelQuantityPairBuilder(root,()=>{if(nelQuantityPairRead(root))reveal();});
    return;
  }
  if(step.kind==='side-choice'||step.kind==='left-relation'||step.kind==='graph-choice'){
    const root=core?.querySelector(step.kind==='graph-choice'?'.nel-quantity-object-graph':'.nel-quantity-relation-choice');
    const answer=step.kind==='side-choice'||step.kind==='graph-choice'?x.language.sideAnswer:x.language.relationLabel;
    bindNelQuantityRelation(root,()=>{
      const value=nelQuantityRelationRead(root);
      if(value===answer){if(help)help.textContent='';reveal();}
      else if(help)help.textContent='Nesneleri bire bir düşün; büyüklük ve aralık miktarı değiştirmez.';
    });
    return;
  }
  if(step.kind==='language-pair'){
    const seen=new Set();
    core?.querySelectorAll('[data-quantity-language]').forEach(button=>button.addEventListener('click',()=>{
      seen.add(button.dataset.quantityLanguage);button.classList.add('selected');
      if(help)help.textContent=seen.size===2?'İki söyleyişi de gördün.':'Diğer söyleyişe de dokun.';
      if(seen.size===2)reveal();
    }));
    return;
  }
  $('#nelQuantityConfirm')?.addEventListener('click',reveal);
}
function completeNelCompareQuantitiesLessonStep(skill,index){
  const ss=ensureSkillState(state,skill.id),lc=ss.learningCycle,next=index+1;lc.lessonStepIndex=Math.max(lc.lessonStepIndex||0,next);lc.lessonVersion=NEL_QUANTITY_COMPARE_LESSON_VERSION;
  if(next>=NEL_QUANTITY_COMPARE_LESSON_STEPS.length){lc.lessonTaughtAt=lc.lessonTaughtAt||Date.now();saveState();session.planIndex++;loadPlanItem();return;}
  saveState();session.lessonStepIndex=next;renderNelCompareQuantitiesLessonStep(skill,next);
}
function renderNelCompareQuantitiesLessonStep(skill,index=null){
  const ss=ensureSkillState(state,skill.id),saved=Math.min(NEL_QUANTITY_COMPARE_LESSON_STEPS.length-1,Math.max(0,ss.learningCycle?.lessonStepIndex||0));
  const at=index==null?(session?.lessonReplayStep!=null?Math.min(NEL_QUANTITY_COMPARE_LESSON_STEPS.length-1,Math.max(0,Number(session.lessonReplayStep)||0)):(session?.lessonReplay?0:saved)):index;
  const step=NEL_QUANTITY_COMPARE_LESSON_STEPS[at];session.lessonStepIndex=at;currentQuestion=null;renderPracticeHeader(skill);$('#practiceMode').textContent='KEŞFET';$('#practiceMode').dataset.mode='teach';$('#practiceCounter').textContent=step.section+' • '+(at+1)+' / '+NEL_QUANTITY_COMPARE_LESSON_STEPS.length;$('#practiceProgress').style.width=Math.round((at+1)/NEL_QUANTITY_COMPARE_LESSON_STEPS.length*100)+'%';
  $('#practiceContent').innerHTML='<div class="nel-quantity-lesson-stage" data-nel-quantity-step="'+esc(step.id)+'">'+nelQuantityCompareSectionTrack(step)+'<div class="lesson-step-copy"><span class="lesson-kicker">'+esc(step.section)+' · '+(at+1)+' / '+NEL_QUANTITY_COMPARE_LESSON_STEPS.length+'</span><h2>'+esc(step.title)+'</h2><p>'+esc(step.body)+'</p></div><div class="nel-quantity-lesson-visual">'+nelQuantityLessonCore(step)+'</div><div class="lesson-step-actions"><button type="button" class="response-submit lesson-next-button" id="nelQuantityLessonNext" disabled>'+(at===NEL_QUANTITY_COMPARE_LESSON_STEPS.length-1?'Sayı hissi yoluna devam':'Sonraki keşif')+' <b>→</b></button></div></div>';
...
const NEL_BASIC_SHAPES_LESSON_VERSION=1;
const NEL_BASIC_SHAPES_SECTIONS=['EŞLEŞTİR','FARKLI GÖR','ADINI DİNLE','ANLAT','ÇEVREDE BUL'];
const NEL_BASIC_SHAPE_NAMES={circle:'daire',square:'kare',rectangle:'dikdörtgen',triangle:'üçgen'};
function nelBasicShapeLessonItem(shape,size='medium',rotation=0,tone='blue',context=null,id=null){
  return {id:id||shape+'-'+size+'-'+rotation,shape,nameTr:NEL_BASIC_SHAPE_NAMES[shape],size,rotation,tone,context};
}
function nelBasicShapeLessonOptions(target,variantIndex=0,environment=false){
  const order=['circle','square','rectangle','triangle'];
...
const NEL_BASIC_SHAPES_LESSON_STEPS=[
  {id:'circle-match',section:'EŞLEŞTİR',kind:'match',target:nelBasicShapeLessonItem('circle','medium',0,'blue'),answer:'circle',variant:1,title:'Daireyi başka görünüşte bul.',body:'Boyutu ve rengi değişse de aynı temel şekli eşleştir.',result:'İki görünüş de daire.'},
  {id:'square-rotate',section:'FARKLI GÖR',kind:'match',target:nelBasicShapeLessonItem('square','medium',0,'amber'),answer:'square',variant:0,title:'Kare dönünce de karedir.',body:'Kareyi farklı yönde gösteren kartı bul.',result:'Kare döndü; şeklin adı değişmedi.'},
  {id:'triangle-turn',section:'FARKLI GÖR',kind:'match',target:nelBasicShapeLessonItem('triangle','large',180,'rose'),answer:'triangle',variant:2,title:'Üçgen her zaman yukarı bakmaz.',body:'Farklı yöndeki üçgeni bul.',result:'Yön değişti; şekil yine üçgen.'},
  {id:'rectangle-turn',section:'FARKLI GÖR',kind:'match',target:nelBasicShapeLessonItem('rectangle','medium',90,'green'),answer:'rectangle',variant:1,title:'Dikdörtgen yatay ya da dikey olabilir.',body:'Aynı dikdörtgen biçimini başka yönde bul.',result:'Yön değişse de dikdörtgen adı aynı kaldı.'},
  {id:'size-change',section:'FARKLI GÖR',kind:'match',target:nelBasicShapeLessonItem('circle','small',0,'green'),answer:'circle',variant:2,title:'Büyüklük şeklin adını değiştirmez.',body:'Küçük daireyle aynı şekli daha büyük görünüşte bul.',result:'Küçük ve büyük olan iki görünüş de daire.'},
  {id:'mixed-invariance',section:'FARKLI GÖR',kind:'match',target:nelBasicShapeLessonItem('square','large',45,'rose'),answer:'square',variant:3,title:'Renk, boyut ve yön birlikte değişebilir.',body:'Bütün görünüş değişikliklerine rağmen kareyi bul.',result:'Görünüş değişti; temel şekil kare kaldı.'},
  {id:'name-circle-square',section:'ADINI DİNLE',kind:'audio',target:nelBasicShapeLessonItem('square','medium',45,'blue'),answer:'square',title:'Şeklin adını dinleyerek bul.',body:'Şekle bak, sesli seçenekleri dinle ve doğru adı seç.',result:'Bu şeklin adı kare.'},
  {id:'name-triangle-rectangle',section:'ADINI DİNLE',kind:'audio',target:nelBasicShapeLessonItem('triangle','medium',60,'amber'),answer:'triangle',title:'Başka bir şeklin adını dinle.',body:'Okumaya ihtiyaç duymadan sesli şekil adını seç.',result:'Bu şeklin adı üçgen.'},
  {id:'explain-invariance',section:'ANLAT',kind:'explain',target:nelBasicShapeLessonItem('rectangle','small',0,'blue'),other:nelBasicShapeLessonItem('rectangle','large',90,'rose'),answer:'same',title:'Neden ikisine de dikdörtgen diyoruz?',body:'Boyut ve yön değiştiğinde temel şeklin adının ne olduğuna karar ver.',result:'Döndürmek veya boyutunu değiştirmek şeklin adını değiştirmez.'},
  {id:'environment-hunt',section:'ÇEVREDE BUL',kind:'environment',target:nelBasicShapeLessonItem('triangle','medium',120,'green'),answer:'triangle',variant:1,title:'Şekli günlük çevrede bul.',body:'Hedefle aynı düz şekli gösteren çevre örneğini seç.',result:'Temel şekilleri çevremizdeki düz yüzlerde de fark edebiliriz.'}
];
function nelBasicShapeSectionTrack(step){return '<div class="nel-basic-shape-section-track">'+NEL_BASIC_SHAPES_SECTIONS.map(name=>'<span class="'+(name===step.section?'active':'')+'">'+esc(name)+'</span>').join('')+'</div>';}
function nelBasicShapeGlyph(item={}){
  const shape=item.shape||'circle',size=item.size||'medium',rotation=Number(item.rotation)||0,tone=item.tone||'blue';
  return '<div class="nel-basic-shape-glyph '+esc(shape)+' '+esc(size)+' tone-'+esc(tone)+'" data-basic-shape="'+esc(shape)+'" data-shape-rotation="'+rotation+'" style="--shape-rotation:'+rotation+'deg" aria-hidden="true"><i></i></div>';
}
function nelBasicShapeCardVisual(v={}){
  const item=v.item||v;
  return '<div class="nel-basic-shape-card" data-shape-card="'+esc(item.shape||'')+'">'+nelBasicShapeGlyph(item)+'</div>';
}
function nelBasicShapeContextVisual(v={}){
  const item=v.item||v;
  return '<div class="nel-basic-shape-context" data-shape-context="'+esc(item.context||'')+'">'+nelBasicShapeGlyph(item)+'<span>'+esc(item.context||'çevre örneği')+'</span></div>';
}
function nelBasicShapePairVisual(v={}){
  return '<div class="nel-basic-shape-pair">'+nelBasicShapeCardVisual({item:v.left})+nelBasicShapeCardVisual({item:v.right})+'</div>';
}
function nelBasicShapeMatchVisual(v={}){
  return '<div class="nel-basic-shape-match">'+
    '<div class="nel-basic-shape-target"><small>HEDEF</small>'+nelBasicShapeGlyph(v.target||{})+'</div>'+
    '<div class="nel-basic-shape-options">'+(v.options||[]).map(item=>'<button type="button" data-basic-shape-choice="'+esc(item.shape)+'">'+nelBasicShapeGlyph(item)+'</button>').join('')+'</div>'+
  '</div>';
}
function nelBasicShapeNameVisual(v={}){
  return '<div class="nel-basic-shape-name">'+nelBasicShapeCardVisual({item:v.item})+
    '<div class="nel-basic-shape-audio-options">'+(v.options||[]).map(option=>'<button type="button" data-basic-shape-name="'+esc(String(option.value||'').replace('shape-',''))+'" data-rote-speech="'+esc(option.speech||'')+'">🔊 <span>Dinle</span></button>').join('')+'</div>'+
  '</div>';
}
function bindNelBasicShapeMatch(root,onChange,blocked=()=>false){
  root?.querySelectorAll('[data-basic-shape-choice]').forEach(button=>button.addEventListener('click',()=>{
    if(blocked())return;
    root.querySelectorAll('[data-basic-shape-choice]').forEach(x=>x.classList.remove('selected'));
    button.classList.add('selected');root.dataset.shapeSelected='shape-'+button.dataset.basicShapeChoice;onChange?.();
  }));
}
function bindNelBasicShapeAudioName(root,onChange,blocked=()=>false){
  if(!root)return;bindNelRoteSpeech(root);
  root.querySelectorAll('[data-basic-shape-name]').forEach(button=>button.addEventListener('click',()=>{
    if(blocked())return;
    root.querySelectorAll('[data-basic-shape-name]').forEach(x=>x.classList.remove('selected'));
    button.classList.add('selected');root.dataset.shapeSelected='shape-'+button.dataset.basicShapeName;onChange?.();
  }));
}
function nelBasicShapeRead(root){return root?.dataset.shapeSelected||null;}
function nelBasicShapeLessonCore(step){
  if(step.kind==='match'){
    return '<div class="nel-basic-shape-lesson-core">'+nelBasicShapeMatchVisual({target:step.target,options:nelBasicShapeLessonOptions(step.target,step.variant,false)})+'<p class="nel-basic-shape-help" id="nelBasicShapeHelp">Hedefle aynı temel şekli seç.</p><div class="nel-basic-shape-result" id="nelBasicShapeResult">'+esc(step.result)+'</div></div>';
  }
  if(step.kind==='audio'){
    const options=Object.keys(NEL_BASIC_SHAPE_NAMES).map(shape=>({value:'shape-'+shape,speech:NEL_BASIC_SHAPE_NAMES[shape]}));
    return '<div class="nel-basic-shape-lesson-core">'+nelBasicShapeNameVisual({item:step.target,options})+'<p class="nel-basic-shape-help" id="nelBasicShapeHelp">Sesli seçenekleri dinle.</p><div class="nel-basic-shape-result" id="nelBasicShapeResult">'+esc(step.result)+'</div></div>';
  }
  if(step.kind==='explain'){
    return '<div class="nel-basic-shape-lesson-core">'+nelBasicShapePairVisual({left:step.target,right:step.other})+'<div class="nel-basic-shape-explain-options"><button type="button" data-shape-explain="same">Döndürmek veya boyutunu değiştirmek şeklin adını değiştirmez.</button><button type="button" data-shape-explain="colour">Rengi değişince başka şekil olur.</button><button type="button" data-shape-explain="turn">Yana dönünce başka şekil olur.</button></div><p class="nel-basic-shape-help" id="nelBasicShapeHelp">İki görünüşü karşılaştır.</p><div class="nel-basic-shape-result" id="nelBasicShapeResult">'+esc(step.result)+'</div></div>';
  }
  const options=nelBasicShapeLessonOptions(step.target,step.variant,true);
  return '<div class="nel-basic-shape-lesson-core"><div class="nel-basic-shape-target"><small>HEDEF</small>'+nelBasicShapeGlyph(step.target)+'</div><div class="nel-basic-shape-environment-grid">'+options.map(item=>'<button type="button" data-shape-environment="'+esc(item.shape)+'">'+nelBasicShapeContextVisual({item})+'</button>').join('')+'</div><p class="nel-basic-shape-help" id="nelBasicShapeHelp">Aynı düz şekli çevrede bul.</p><div class="nel-basic-shape-result" id="nelBasicShapeResult">'+esc(step.result)+'</div></div>';
}
function wireNelBasicShapesLessonStep(step,next){
  const root=$('.nel-basic-shape-lesson-core'),help=$('#nelBasicShapeHelp'),result=$('#nelBasicShapeResult');
  const finish=()=>{result?.classList.add('revealed');if(help)help.textContent='';next.disabled=false;};
  if(step.kind==='match'){
    bindNelBasicShapeMatch(root,()=>{
      const value=nelBasicShapeRead(root);
      if(value==='shape-'+step.answer)finish();else if(value)help.textContent='Renk, boyut veya yöne değil temel şekle yeniden bak.';
    });
    return;
  }
  if(step.kind==='audio'){
    bindNelBasicShapeAudioName(root,()=>{
      const value=nelBasicShapeRead(root);
      if(value==='shape-'+step.answer)finish();else if(value)help.textContent='Şekle yeniden bak ve adları tekrar dinle.';
    });
    return;
  }
  if(step.kind==='explain'){
    root?.querySelectorAll('[data-shape-explain]').forEach(button=>button.addEventListener('click',()=>{
      if(button.dataset.shapeExplain===step.answer){button.classList.add('selected');finish();}
      else help.textContent='İki şeklin yönü ve boyutu değişmiş olabilir; temel şeklin adı değişti mi?';
    }));return;
  }
  root?.querySelectorAll('[data-shape-environment]').forEach(button=>button.addEventListener('click',()=>{
    if(button.dataset.shapeEnvironment===step.answer){button.classList.add('selected');finish();}
    else help.textContent='Nesnenin ne olduğuna değil, gördüğün düz şekle bak.';
  }));
}
function completeNelBasicShapesLessonStep(skill,index){
  const ss=ensureSkillState(state,skill.id),lc=ss.learningCycle,next=index+1;lc.lessonStepIndex=Math.max(lc.lessonStepIndex||0,next);lc.lessonVersion=NEL_BASIC_SHAPES_LESSON_VERSION;
  if(next>=NEL_BASIC_SHAPES_LESSON_STEPS.length){lc.lessonTaughtAt=lc.lessonTaughtAt||Date.now();saveState();session.planIndex++;loadPlanItem();return;}saveState();session.lessonStepIndex=next;renderNelBasicShapesLessonStep(skill,next);
}
function renderNelBasicShapesLessonStep(skill,index=null){
  const ss=ensureSkillState(state,skill.id),saved=Math.min(NEL_BASIC_SHAPES_LESSON_STEPS.length-1,Math.max(0,ss.learningCycle?.lessonStepIndex||0));
  const at=index==null?(session?.lessonReplayStep!=null?Math.min(NEL_BASIC_SHAPES_LESSON_STEPS.length-1,Math.max(0,Number(session.lessonReplayStep)||0)):(session?.lessonReplay?0:saved)):index;
  const step=NEL_BASIC_SHAPES_LESSON_STEPS[at];session.lessonStepIndex=at;currentQuestion=null;renderPracticeHeader(skill);$('#practiceMode').textContent='KEŞFET';$('#practiceMode').dataset.mode='teach';$('#practiceCounter').textContent=step.section+' • '+(at+1)+' / '+NEL_BASIC_SHAPES_LESSON_STEPS.length;$('#practiceProgress').style.width=Math.round((at+1)/NEL_BASIC_SHAPES_LESSON_STEPS.length*100)+'%';
  $('#practiceContent').innerHTML='<div class="nel-basic-shape-lesson-stage" data-nel-basic-shape-step="'+esc(step.id)+'">'+nelBasicShapeSectionTrack(step)+'<div class="lesson-step-copy"><span class="lesson-kicker">'+esc(step.section)+' · '+(at+1)+' / '+NEL_BASIC_SHAPES_LESSON_STEPS.length+'</span><h2>'+esc(step.title)+'</h2><p>'+esc(step.body)+'</p></div><div class="nel-basic-shape-lesson-visual">'+nelBasicShapeLessonCore(step)+'</div><div class="lesson-step-actions"><button type="button" class="response-submit lesson-next-button" id="nelBasicShapeLessonNext" disabled>'+(at===NEL_BASIC_SHAPES_LESSON_STEPS.length-1?'Şekil yoluna devam':'Sonraki keşif')+' <b>→</b></button></div></div>';
...
function updateManipulatorStatus(q){
  ...
  else if(['nel-numeral-material-form','nel-numeral-guided-trace','nel-numeral-free-write','nel-numeral-context-record'].includes(q.response?.interaction)){
...
function renderVisual(v,q){
  ...
  switch(v.type){
    ...
    case 'nel-number-quantity': return nelNumberQuantityVisual(v);
    case 'nel-number-link-builder': return nelNumberLinkBuilderVisual(v);
    case 'nel-number-quantity-focus': return nelNumberQuantityFocusVisual(v);
    case 'nel-number-form-match': return nelNumberFormMatchVisual(v);
    case 'nel-number-equivalent-set': return nelNumberEquivalentSetVisual(v);
    case 'nel-number-context-tag': return nelNumberContextTagVisual(v);
    case 'nel-numeral-material-form': return nelNumeralMaterialVisual(v);
    case 'nel-numeral-draw-board': return nelNumeralBoardVisual(v);
    case 'nel-numeral-written-record': return nelNumeralWrittenRecordVisual(v);
    case 'nel-quantity-pair-builder': return nelQuantityPairBuilderVisual(v);
    case 'nel-quantity-set-pair': return nelQuantitySetPairVisual(v);
    ...
    case 'nel-basic-shape-card': return nelBasicShapeCardVisual(v);
    case 'nel-basic-shape-context': return nelBasicShapeContextVisual(v);
